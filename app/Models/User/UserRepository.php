...
class UserRepository
{
      ...
      public function list($request)
      {
         ...
         if (!empty($request) && $request['field'] == 'role_name') {
            $request['field'] = 'roles.name';
         }
         
         $model = $this->model->select('users.id','users.name','users.username','users.email','users.status','roles.name as role_name')
                     ->where('users.id','<>',Access::DEVELOPER)
                     ->leftJoin('roles','roles.id','=','users.role_id')
                     ->searchOrder($request, ['users.name','users.username','users.email','status','roles.name'])
                     ->paginate($limit);
